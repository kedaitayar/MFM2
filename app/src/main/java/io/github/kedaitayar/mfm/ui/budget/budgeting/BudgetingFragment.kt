...
//                            .setAnchorView(binding)
    ...
    private fun updateNotBudgetedValue() {
        val budgetingAmountMonthly =
            budgetingViewModel.budgetingAmountListMonthly.map { it.value.toDoubleOrNull() ?: 0.0 }.sumByDouble { it }
        val budgetingAmountYearly =
            budgetingViewModel.budgetingAmountListYearly.map { it.value.toDoubleOrNull() ?: 0.0 }.sumByDouble { it }
        val totalUnbudgeted =
